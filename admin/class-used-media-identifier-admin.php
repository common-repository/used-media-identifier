 ...
 */
class Used_Media_Identifier_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in Used_Media_Identifier_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Used_Media_Identifier_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/used-media-identifier-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/used-media-identifier-admin.js', array( 'jquery' ), $this->version, false );

	}

        public function remove_wp_print_media_templates(){
            remove_action( 'admin_footer', 'wp_print_media_templates',10);
            remove_action( 'wp_footer', 'wp_print_media_templates', 10 );
            remove_action( 'customize_controls_print_footer_scripts', 'wp_print_media_templates',10 );
            add_action( 'admin_footer', array( $this, 'new_wp_print_media_templates'),10 );
            add_action( 'wp_footer', array( $this, 'new_wp_print_media_templates'),10 );
            add_action( 'customize_controls_print_footer_scripts', array( $this, 'new_wp_print_media_templates'),10 );
            add_filter('wp_prepare_attachment_for_js', array( $this, 'update_media_attachment_url'),10,2);
	}
        
        public function update_image_parent_when($post_ID, $post_after, $post_before){
            global $wpdb;
            $wpdb->update( $wpdb->prefix.'posts', 
                    array( 'post_parent' => '0' ), 
                    array( 'post_parent' => $post_ID, 'post_type' => 'attachment' )
                ); 
            $post_thumb_id = get_post_thumbnail_id($post_ID);

            $parent = wp_get_post_parent_id( $post_thumb_id );
            if($parent == 0){
                wp_update_post(
                    array(
                        'ID' => $post_thumb_id, 
        ...
        public function update_media_attachment_url($attachment,$meta = 'image'){
            
            $upload = wp_upload_dir();
            $mediafileurl = $upload['baseurl']. '/media_labels/';
            
            $medialabelOptGet = get_option('media_file_label_images');
            $postid = $attachment['uploadedTo'];
            $post_type = trim(get_post_type($postid));
            ...
            }else if(in_array($post_type, $allposttypes)){
                 $labelimg = trailingslashit(plugin_dir_url('')).$this->plugin_name.'/images/badge.png';
            }else{
                $labelimg = '';
            }
            $hidelabel = get_option('hide_label_on_image');
            if(!in_array($post_type, array_keys($hidelabel))){
                $attachment['is_badge'] = $labelimg;
            }
            $attachment['dateymd'] = date('Y-m-d', $attachment['date']/1000);
            return $attachment;
	}
        
        public function new_wp_print_media_templates() {
                global $is_IE;
                        ...
                        <div class="attachment-preview js--select-attachment type-{{ data.type }} subtype-{{ data.subtype }} {{ data.orientation }}">
                                <div class="thumbnail">
                                        ...
                                        <# } else if ( 'image' === data.type && data.sizes ) { #>
                                                ...
                                                <div class="badge">
                                                <# if ( data.is_badge ) { #>
                                                    <img src="{{ data.is_badge }}" class="badge" draggable="false" alt="" />
                                                <# } #>
                                                </div>
                                                <div class="imgdate">{{ data.dateymd }}</div>
                                        <# } else { #>
                                                <div class="centered">
